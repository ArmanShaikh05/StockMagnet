...
const EditProductDialog = ({
  ...
  productsData: SerializedSingleProduct;
}) => {
  ...
  const [productSno, setProductSno] = useState<string>(productsData.productSno);
  const [productBrandId, setProductBrandId] = useState<string>(
    productsData.brandId
  );
  const [productMrp, setProductMrp] = useState<number>(
    Number(productsData.MRP)
  );
  const [purchasePrice, setPurchasePrice] = useState<number>(
    Number(productsData.purchasePrice)
  );
  const [hsnCode, setHsnCode] = useState<string>(productsData.HSNCode || "");
  const [taxRate, setTaxRate] = useState<string>(productsData.taxRate || "");
  const [taxInclusion, setTaxInclusion] = useState<string>(
    productsData.taxIncludedWithMrp === true ? "included" : "excluded"
  );
  const [warranty, setWarranty] = useState<number>(
    Number(productsData.warrantyMonths) || 0
  );
  const [stockInHand, setStockInHand] = useState<number>(
    Number(productsData.stockInHand)
  );
  const [minStockQty, setMinStockQty] = useState<number>(
    Number(productsData.minStockQty)
  );
  const [branch, setBranch] = useState<string>(productsData.branchId);
  const [unitId, setUnitId] = useState<string>(productsData.unitId);
  const [reorderQty, setReorderQty] = useState<number>(
    Number(productsData.reorderQty) || 0
  );
  const [categoryId, setCategoryId] = useState<string>(productsData.categoryId);
  const [tags, setTags] = useState<string[]>(productsData.tags || []);
  ...
  const handleEditProduct = async () => {
    try {
      ...
      let productImageUrl = productsData.productImage!;
      let productImageId = productsData.imageId!;
      ...
      const response: { success: boolean; message: string } = await editProduct(
        { data: productData, productId: productsData.id }
      );
  ...
  return (
    <div className="lg:px-16">
      ...
      <div className="flex flex-col w-full md:flex-row md:items-center md:justify-end gap-4 mt-6">
        <Button disabled={loading} onClick={() => handleEditProduct()}>
          {loading ? (
