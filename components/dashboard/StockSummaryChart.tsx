...
const StockSummaryChart = () => {
  ...
  useEffect(() => {
    (async () => {
      if (selectedBranch) {
        setLoading(true);
        const response = await getMonthStockSummary(selectedBranch.id);
    ...
    <Card className="flex flex-col shadow-xl">
      ...
      <CardFooter className="flex-col gap-2 text-sm">
        <div className="flex items-center gap-2 font-medium leading-none">
          ...
          {Number(percentChange.slice(0,-1) || 0) > 0 ? (<TrendingUp className="h-4 w-4 text-green-500"  />) : (<TrendingDown className="h-4 w-4 text-red-500" />)}
